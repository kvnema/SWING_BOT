"""
SWING_BOT Configuration and Instrument Definitions
Centralized configuration to eliminate code duplication and improve maintainability
"""

import os
import requests
import pandas as pd
from typing import Dict, List
from dotenv import load_dotenv

# Load environment variables from .env file
load_dotenv()

def fetch_nifty200_stocks() -> List[str]:
    """Fetch NIFTY 200 stock list from NSE website."""
    try:
        url = "https://archives.nseindia.com/content/indices/ind_nifty200list.csv"
        response = requests.get(url, timeout=30)
        response.raise_for_status()
        
        # Parse CSV
        df = pd.read_csv(pd.io.common.StringIO(response.text))
        symbols = df['Symbol'].dropna().unique().tolist()
        
        # Add .NS suffix
        symbols = [f"{symbol}.NS" for symbol in symbols]
        
        print(f"Successfully fetched {len(symbols)} NIFTY 200 stocks")
        return symbols
        
    except Exception as e:
        print(f"Failed to fetch NIFTY 200 list: {e}")
        # Fallback to hardcoded list (first 200 from NIFTY 500)
        return NIFTY_500_STOCKS[:200]

def fetch_nse_etfs() -> List[str]:
    """Fetch complete NSE ETF list - currently using comprehensive hardcoded list."""
    # Comprehensive NSE ETF list (can be updated periodically)
    etfs = [
        'ABSLBANETF.NS', 'ALPHAETF.NS', 'ALPL30IETF.NS', 'AUTOBEES.NS', 'AUTOIETF.NS',
        'AXISBNKETF.NS', 'AXISBPSETF.NS', 'AXISCETF.NS', 'AXISGOLD.NS', 'AXISHCETF.NS',
        'AXISTECETF.NS', 'BANKBEES.NS', 'BANKBETF.NS', 'BANKETF.NS', 'BANKIETF.NS',
        'BANKNIFTY1.NS', 'BBETF0432.NS', 'BBNPNBETF.NS', 'BFSI.NS', 'BSE500IETF.NS',
        'BSLGOLDETF.NS', 'BSLNIFTY.NS', 'BSLSENETFG.NS', 'CASHIETF.NS', 'COMMOIETF.NS',
        'CONSUMBEES.NS', 'CONSUMIETF.NS', 'CPSEETF.NS', 'EBBETF0430.NS', 'EBBETF0431.NS',
        'EBBETF0433.NS', 'EQUAL50ADD.NS', 'ESG.NS', 'EVIETF.NS', 'FINIETF.NS',
        'FMCGIETF.NS', 'GILT5YBEES.NS', 'GILTIETF.NS', 'GOLD1.NS', 'GOLDBEES.NS',
        'GOLDETF.NS', 'GOLDIETF.NS', 'GOLDSHARE.NS', 'GSEC10IETF.NS', 'GSEC10YEAR.NS',
        'HANGSENG BEES-NAV.NS', 'HDFCGOLD.NS', 'HEALTHIETF.NS', 'HNGSNGBEES.NS', 'INFRAIETF.NS',
        'ITBEES.NS', 'ITIETF.NS', 'ITETF.NS', 'IVZINGOLD.NS', 'IVZINNIFTY.NS',
        'JETFREIGHT.NS', 'JUNIORBEES.NS', 'LICMFGOLD.NS', 'LICNETFGSC.NS', 'LICNETFN50.NS',
        'LICNETFSEN.NS', 'LIQGRWBEES.NS', 'LIQUIDBEES.NS', 'LIQUIDBETF.NS', 'LIQUIDETF.NS',
        'LIQUIDIETF.NS', 'LOWVOLIETF.NS', 'LTGILTBEES.NS', 'MAKEINDIA.NS', 'MANUFGBEES.NS',
        'METALIETF.NS', 'MID150BEES.NS', 'MIDCAPETF.NS', 'MIDCAPIETF.NS', 'MIDQ50ADD.NS',
        'MIDSELIETF.NS', 'MOM100.NS', 'MOM30IETF.NS', 'MOM50.NS', 'NETF.NS',
        'NEXT50IETF.NS', 'NIF100BEES.NS', 'NIF100IETF.NS', 'NIF10GETF.NS', 'NIF5GETF.NS',
        'NIFITETF.NS', 'NIFTY1.NS', 'NIFTYBETF.NS', 'NIFTYBEES.NS', 'NIFTYIETF.NS',
        'NIFTYETF.NS', 'NV20.NS', 'NV20BEES.NS', 'NV20IETF.NS', 'OILIETF.NS',
        'PHARMABEES.NS', 'PSUBANKADD.NS', 'PSUBNKBEES.NS', 'PSUBNKIETF.NS', 'PVTBANKADD.NS',
        'PVTBANIETF.NS', 'QNIFTY.NS', 'QUAL30IETF.NS', 'SBIETFCON.NS', 'SBIETFIT.NS',
        'SBIETFPB.NS', 'SBIETFQLTY.NS', 'SBILIQETF.NS', 'SBINEQWETF.NS', 'SDL26BEES.NS',
        'SENSEXADD.NS', 'SENSEXETF.NS', 'SENSEXIETF.NS', 'SETF10GILT.NS', 'SETFGOLD.NS',
        'SETFNIF50.NS', 'SETFNIFBK.NS', 'SETFNN50.NS', 'SHARIABEES.NS', 'SILVERADD.NS',
        'SILVERBEES.NS', 'SILVERETF.NS', 'SILVERIETF.NS', 'SNXT30BEES.NS', 'TNIDETF.NS',
        'TOP15IETF.NS', 'UTIBANKETF.NS', 'UTINIFTETF.NS', 'UTISENSETF.NS', 'VAL30IETF.NS'
    ]
    print(f"Using {len(etfs)} NSE ETFs")
    return etfs

# Upstox API credentials from .env
API_KEY = os.getenv('UPSTOX_API_KEY')
API_SECRET = os.getenv('UPSTOX_API_SECRET')
ACCESS_TOKEN = os.getenv('UPSTOX_ACCESS_TOKEN')

# Angel One API credentials from .env
ANGEL_ONE_API_KEY = os.getenv('ANGEL_ONE_API_KEY')
ANGEL_ONE_CLIENT_ID = os.getenv('ANGEL_ONE_CLIENT_ID')
ANGEL_ONE_PASSWORD = os.getenv('ANGEL_ONE_PASSWORD')
ANGEL_ONE_TOTP_SECRET = os.getenv('ANGEL_ONE_TOTP_SECRET')
ANGEL_ONE_JWT_TOKEN = os.getenv('ANGEL_ONE_JWT_TOKEN')
ANGEL_ONE_FEED_TOKEN = os.getenv('ANGEL_ONE_FEED_TOKEN')

# Telegram Bot Configuration for alerts
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN')
TELEGRAM_CHAT_ID = os.getenv('TELEGRAM_CHAT_ID')
TELEGRAM_ENABLED = bool(TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID)

# Zerodha/Kite API Configuration for live trading
KITE_API_KEY = os.getenv('KITE_API_KEY')
KITE_API_SECRET = os.getenv('KITE_API_SECRET')
KITE_ACCESS_TOKEN = os.getenv('KITE_ACCESS_TOKEN')
KITE_PUBLIC_TOKEN = os.getenv('KITE_PUBLIC_TOKEN')
KITE_ENABLED = bool(KITE_API_KEY and KITE_API_SECRET and KITE_ACCESS_TOKEN)

# Rate limiting and performance settings
API_RATE_LIMIT_DELAY = 0.1  # seconds between API calls
MAX_RETRIES = 3
RETRY_BACKOFF_FACTOR = 2

# Data processing settings
MIN_DATA_POINTS = 30  # Minimum data points required for indicator calculation
DEFAULT_LOOKBACK_DAYS = 500

# Instrument keys for Upstox (NSE_EQ and NSE_INDEX)
INSTRUMENT_KEYS: Dict[str, str] = {
    # NIFTY 500 Stocks
    '360ONE': 'NSE_EQ|INE466L01038',
    '3MINDIA': 'NSE_EQ|INE470A01017',
    'ABB': 'NSE_EQ|INE117A01022',
    'ACC': 'NSE_EQ|INE012A01025',
    'AIAENG': 'NSE_EQ|INE212H01026',
    'APLAPOLLO': 'NSE_EQ|INE702C01027',
    'AUBANK': 'NSE_EQ|INE949L01017',
    'AARTIIND': 'NSE_EQ|INE769A01020',
    'AAVAS': 'NSE_EQ|INE216P01012',
    'ABBOTINDIA': 'NSE_EQ|INE358A01014',
    'ACE': 'NSE_EQ|INE731H01025',
    'ADANIENSOL': 'NSE_EQ|INE931S01010',
    'ADANIENT': 'NSE_EQ|INE423A01024',
    'ADANIGREEN': 'NSE_EQ|INE364U01010',
    'ADANIPORTS': 'NSE_EQ|INE742F01042',
    'ADANIPOWER': 'NSE_EQ|INE814H01011',
    'ATGL': 'NSE_EQ|INE399L01023',
    'AWL': 'NSE_EQ|INE699H01024',
    'ABCAPITAL': 'NSE_EQ|INE674K01013',
    'ABFRL': 'NSE_EQ|INE647O01011',
    'AEGISLOG': 'NSE_EQ|INE208C01025',
    'AETHER': 'NSE_EQ|INE0BWX01014',
    'AFFLE': 'NSE_EQ|INE00WC01027',
    'AJANTPHARM': 'NSE_EQ|INE031B01049',
    'APLLTD': 'NSE_EQ|INE901L01018',
    'ALKEM': 'NSE_EQ|INE540L01014',
    'ALKYLAMINE': 'NSE_EQ|INE150B01039',
    'ALLCARGO': 'NSE_EQ|INE418H01029',
    'ALOKINDS': 'NSE_EQ|INE270A01029',
    'ARE&M': 'NSE_EQ|INE885A01032',
    'AMBER': 'NSE_EQ|INE371P01015',
    'AMBUJACEM': 'NSE_EQ|INE079A01024',
    'ANANDRATHI': 'NSE_EQ|INE463V01026',
    'ANGELONE': 'NSE_EQ|INE732I01013',
    'ANURAS': 'NSE_EQ|INE930P01018',
    'APARINDS': 'NSE_EQ|INE372A01015',
    'APOLLOHOSP': 'NSE_EQ|INE437A01024',
    'APOLLOTYRE': 'NSE_EQ|INE438A01022',
    'APTUS': 'NSE_EQ|INE852O01025',
    'ACI': 'NSE_EQ|INE128X01021',
    'ASAHIINDIA': 'NSE_EQ|INE439A01020',
    'ASHOKLEY': 'NSE_EQ|INE208A01029',
    'ASIANPAINT': 'NSE_EQ|INE021A01026',
    'ASTERDM': 'NSE_EQ|INE914M01019',
    'ASTRAZEN': 'NSE_EQ|INE203A01020',
    'ASTRAL': 'NSE_EQ|INE006I01046',
    'ATUL': 'NSE_EQ|INE100A01010',
    'AUROPHARMA': 'NSE_EQ|INE406A01037',
    'AVANTIFEED': 'NSE_EQ|INE871C01038',
    'DMART': 'NSE_EQ|INE192R01011',
    'AXISBANK': 'NSE_EQ|INE238A01034',
    'BEML': 'NSE_EQ|INE258A01016',
    'BLS': 'NSE_EQ|INE153T01027',
    'BSE': 'NSE_EQ|INE118H01025',
    'BAJAJ-AUTO': 'NSE_EQ|INE917I01010',
    'BAJFINANCE': 'NSE_EQ|INE296A01024',
    'BAJAJFINSV': 'NSE_EQ|INE918I01026',
    'BAJAJHLDNG': 'NSE_EQ|INE118A01012',
    'BALAMINES': 'NSE_EQ|INE050E01027',
    'BALKRISIND': 'NSE_EQ|INE787D01026',
    'BALRAMCHIN': 'NSE_EQ|INE119A01028',
    'BANDHANBNK': 'NSE_EQ|INE545U01014',
    'BANKBARODA': 'NSE_EQ|INE028A01039',
    'BANKINDIA': 'NSE_EQ|INE084A01016',
    'MAHABANK': 'NSE_EQ|INE457A01014',
    'BATAINDIA': 'NSE_EQ|INE176A01028',
    'BAYERCROP': 'NSE_EQ|INE462A01022',
    'BERGEPAINT': 'NSE_EQ|INE463A01038',
    'BDL': 'NSE_EQ|INE171Z01026',
    'BEL': 'NSE_EQ|INE263A01024',
    'BHARATFORG': 'NSE_EQ|INE465A01025',
    'BHEL': 'NSE_EQ|INE257A01026',
    'BPCL': 'NSE_EQ|INE029A01011',
    'BHARTIARTL': 'NSE_EQ|INE397D01024',
    'BIKAJI': 'NSE_EQ|INE00E101023',
    'BIOCON': 'NSE_EQ|INE376G01013',
    'BIRLACORPN': 'NSE_EQ|INE340A01012',
    'BSOFT': 'NSE_EQ|INE836A01035',
    'BLUEDART': 'NSE_EQ|INE233B01017',
    'BLUESTARCO': 'NSE_EQ|INE472A01039',
    'BBTC': 'NSE_EQ|INE050A01025',
    'BORORENEW': 'NSE_EQ|INE666D01022',
    'BOSCHLTD': 'NSE_EQ|INE323A01026',
    'BRIGADE': 'NSE_EQ|INE791I01019',
    'BRITANNIA': 'NSE_EQ|INE216A01030',
    'MAPMYINDIA': 'NSE_EQ|INE0BV301023',
    'CCL': 'NSE_EQ|INE421D01022',
    'CESC': 'NSE_EQ|INE486A01021',
    'CGPOWER': 'NSE_EQ|INE067A01029',
    'CIEINDIA': 'NSE_EQ|INE536H01010',
    'CRISIL': 'NSE_EQ|INE007A01025',
    'CSBBANK': 'NSE_EQ|INE679A01013',
    'CAMPUS': 'NSE_EQ|INE278Y01022',
    'CANFINHOME': 'NSE_EQ|INE477A01020',
    'CANBK': 'NSE_EQ|INE476A01022',
    'CAPLIPOINT': 'NSE_EQ|INE475E01026',
    'CGCL': 'NSE_EQ|INE180C01042',
    'CARBORUNIV': 'NSE_EQ|INE120A01034',
    'CASTROLIND': 'NSE_EQ|INE172A01027',
    'CEATLTD': 'NSE_EQ|INE482A01020',
    'CELLO': 'NSE_EQ|INE0LMW01024',
    'CENTRALBK': 'NSE_EQ|INE483A01010',
    'CDSL': 'NSE_EQ|INE736A01011',
    'CENTURYPLY': 'NSE_EQ|INE348B01021',
    'ABREL': 'NSE_EQ|INE055A01016',
    'CERA': 'NSE_EQ|INE739E01017',
    'CHALET': 'NSE_EQ|INE427F01016',
    'CHAMBLFERT': 'NSE_EQ|INE085A01013',
    'CHEMPLASTS': 'NSE_EQ|INE488A01050',
    'CHENNPETRO': 'NSE_EQ|INE178A01016',
    'CHOLAHLDNG': 'NSE_EQ|INE149A01033',
    'CHOLAFIN': 'NSE_EQ|INE121A01024',
    'CIPLA': 'NSE_EQ|INE059A01026',
    'CUB': 'NSE_EQ|INE491A01021',
    'CLEAN': 'NSE_EQ|INE227W01023',
    'COALINDIA': 'NSE_EQ|INE522F01014',
    'COCHINSHIP': 'NSE_EQ|INE704P01025',
    'COFORGE': 'NSE_EQ|INE591G01017',
    'COLPAL': 'NSE_EQ|INE259A01022',
    'CAMS': 'NSE_EQ|INE596I01012',
    'CONCORDBIO': 'NSE_EQ|INE338H01029',
    'CONCOR': 'NSE_EQ|INE111A01025',
    'COROMANDEL': 'NSE_EQ|INE169A01031',
    'CRAFTSMAN': 'NSE_EQ|INE00LO01017',
    'CREDITACC': 'NSE_EQ|INE741K01010',
    'CROMPTON': 'NSE_EQ|INE299U01018',
    'CUMMINSIND': 'NSE_EQ|INE298A01020',
    'CYIENT': 'NSE_EQ|INE136B01020',
    'DCMSHRIRAM': 'NSE_EQ|INE499A01024',
    'DLF': 'NSE_EQ|INE271C01023',
    'DOMS': 'NSE_EQ|INE321T01012',
    'DABUR': 'NSE_EQ|INE016A01026',
    'DALBHARAT': 'NSE_EQ|INE00R701025',
    'DATAPATTNS': 'NSE_EQ|INE0IX101010',
    'DEEPAKFERT': 'NSE_EQ|INE501A01019',
    'DEEPAKNTR': 'NSE_EQ|INE288B01029',
    'DELHIVERY': 'NSE_EQ|INE148O01028',
    'DEVYANI': 'NSE_EQ|INE872J01023',
    'DIVISLAB': 'NSE_EQ|INE361B01024',
    'DIXON': 'NSE_EQ|INE935N01020',
    'LALPATHLAB': 'NSE_EQ|INE600L01024',
    'DRREDDY': 'NSE_EQ|INE089A01023',
    'EIDPARRY': 'NSE_EQ|INE126A01031',
    'EIHOTEL': 'NSE_EQ|INE230A01023',
    'EPL': 'NSE_EQ|INE255A01020',
    'EASEMYTRIP': 'NSE_EQ|INE07O001026',
    'EICHERMOT': 'NSE_EQ|INE066A01021',
    'ELECON': 'NSE_EQ|INE205B01023',
    'ELGIEQUIP': 'NSE_EQ|INE285A01027',
    'EMAMILTD': 'NSE_EQ|INE548C01032',
    'ENDURANCE': 'NSE_EQ|INE913H01037',
    'ENGINERSIN': 'NSE_EQ|INE510A01028',
    'EQUITASBNK': 'NSE_EQ|INE063P01018',
    'ERIS': 'NSE_EQ|INE406M01024',
    'ESCORTS': 'NSE_EQ|INE042A01014',
    'EXIDEIND': 'NSE_EQ|INE302A01020',
    'FDC': 'NSE_EQ|INE258B01022',
    'NYKAA': 'NSE_EQ|INE388Y01029',
    'FEDERALBNK': 'NSE_EQ|INE171A01029',
    'FACT': 'NSE_EQ|INE188A01015',
    'FINEORG': 'NSE_EQ|INE686Y01026',
    'FINCABLES': 'NSE_EQ|INE235A01022',
    'FINPIPE': 'NSE_EQ|INE183A01024',
    'FSL': 'NSE_EQ|INE684F01012',
    'FIVESTAR': 'NSE_EQ|INE128S01021',
    'FORTIS': 'NSE_EQ|INE061F01013',
    'GAIL': 'NSE_EQ|INE129A01019',
    'GMMPFAUDLR': 'NSE_EQ|INE541A01023',
    'GMRINFRA': 'NSE_EQ|INE776C01039',
    'GRSE': 'NSE_EQ|INE382Z01011',
    'GICRE': 'NSE_EQ|INE481Y01014',
    'GILLETTE': 'NSE_EQ|INE322A01010',
    'GLAND': 'NSE_EQ|INE068V01023',
    'GLAXO': 'NSE_EQ|INE159A01016',
    'ALIVUS': 'NSE_EQ|INE03Q201024',
    'GLENMARK': 'NSE_EQ|INE935A01035',
    'MEDANTA': 'NSE_EQ|INE474Q01031',
    'GPIL': 'NSE_EQ|INE177H01021',
    'GODFRYPHLP': 'NSE_EQ|INE260B01028',
    'GODREJCP': 'NSE_EQ|INE102D01028',
    'GODREJIND': 'NSE_EQ|INE233A01035',
    'GODREJPROP': 'NSE_EQ|INE484J01027',
    'GRANULES': 'NSE_EQ|INE101D01020',
    'GRAPHITE': 'NSE_EQ|INE371A01025',
    'GRASIM': 'NSE_EQ|INE047A01021',
    'GESHIP': 'NSE_EQ|INE017A01032',
    'GRINDWELL': 'NSE_EQ|INE536A01023',
    'GAEL': 'NSE_EQ|INE036B01030',
    'FLUOROCHEM': 'NSE_EQ|INE09N301011',
    'GUJGASLTD': 'NSE_EQ|INE844O01030',
    'GMDCLTD': 'NSE_EQ|INE131A01031',
    'GNFC': 'NSE_EQ|INE113A01013',
    'GPPL': 'NSE_EQ|INE517F01014',
    'GSFC': 'NSE_EQ|INE026A01025',
    'GSPL': 'NSE_EQ|INE246F01010',
    'HEG': 'NSE_EQ|INE545A01016',
    'HBLENGINE': 'NSE_EQ|INE292B01021',
    'HCLTECH': 'NSE_EQ|INE860A01027',
    'HDFCAMC': 'NSE_EQ|INE127D01025',
    'HDFCBANK': 'NSE_EQ|INE040A01034',
    'HDFCLIFE': 'NSE_EQ|INE795G01014',
    'HFCL': 'NSE_EQ|INE548A01028',
    'HAPPSTMNDS': 'NSE_EQ|INE419U01012',
    'HAPPYFORGE': 'NSE_EQ|INE330T01021',
    'HAVELLS': 'NSE_EQ|INE176B01034',
    'HEROMOTOCO': 'NSE_EQ|INE158A01026',
    'HSCL': 'NSE_EQ|INE019C01026',
    'HINDALCO': 'NSE_EQ|INE038A01020',
    'HAL': 'NSE_EQ|INE066F01020',
    'HINDCOPPER': 'NSE_EQ|INE531E01026',
    'HINDPETRO': 'NSE_EQ|INE094A01015',
    'HINDUNILVR': 'NSE_EQ|INE030A01027',
    'HINDZINC': 'NSE_EQ|INE267A01025',
    'POWERINDIA': 'NSE_EQ|INE07Y701011',
    'HOMEFIRST': 'NSE_EQ|INE481N01025',
    'HONASA': 'NSE_EQ|INE0J5401028',
    'HONAUT': 'NSE_EQ|INE671A01010',
    'HUDCO': 'NSE_EQ|INE031A01017',
    'ICICIBANK': 'NSE_EQ|INE090A01021',
    'ICICIGI': 'NSE_EQ|INE765G01017',
    'ICICIPRULI': 'NSE_EQ|INE726G01019',
    'ISEC': 'NSE_EQ|INE763G01038',
    'IDBI': 'NSE_EQ|INE008A01015',
    'IDFCFIRSTB': 'NSE_EQ|INE092T01019',
    'IFCI': 'NSE_EQ|INE039A01010',
    'IIFL': 'NSE_EQ|INE530B01024',
    'IRB': 'NSE_EQ|INE821I01022',
    'IRCON': 'NSE_EQ|INE962Y01021',
    'ITC': 'NSE_EQ|INE154A01025',
    'ITI': 'NSE_EQ|INE248A01017',
    'INDIACEM': 'NSE_EQ|INE383A01012',
    'INDIAMART': 'NSE_EQ|INE933S01016',
    'INDIANB': 'NSE_EQ|INE562A01011',
    'IEX': 'NSE_EQ|INE022Q01020',
    'INDHOTEL': 'NSE_EQ|INE053A01029',
    'IOC': 'NSE_EQ|INE242A01010',
    'IOB': 'NSE_EQ|INE565A01014',
    'IRCTC': 'NSE_EQ|INE335Y01020',
    'IRFC': 'NSE_EQ|INE053F01010',
    'INDIGOPNTS': 'NSE_EQ|INE09VQ01012',
    'IGL': 'NSE_EQ|INE203G01027',
    'INDUSTOWER': 'NSE_EQ|INE121J01017',
    'INDUSINDBK': 'NSE_EQ|INE095A01012',
    'NAUKRI': 'NSE_EQ|INE663F01024',
    'INFY': 'NSE_EQ|INE009A01021',
    'INOXWIND': 'NSE_EQ|INE066P01011',
    'INTELLECT': 'NSE_EQ|INE306R01017',
    'INDIGO': 'NSE_EQ|INE646L01027',
    'IPCALAB': 'NSE_EQ|INE571A01038',
    'JBCHEPHARM': 'NSE_EQ|INE572A01036',
    'JKCEMENT': 'NSE_EQ|INE823G01014',
    'JBMA': 'NSE_EQ|INE927D01044',
    'JKLAKSHMI': 'NSE_EQ|INE786A01032',
    'JKPAPER': 'NSE_EQ|INE789E01012',
    'JMFINANCIL': 'NSE_EQ|INE780C01023',
    'JSWENERGY': 'NSE_EQ|INE121E01018',
    'JSWINFRA': 'NSE_EQ|INE880J01026',
    'JSWSTEEL': 'NSE_EQ|INE019A01038',
    'JAI': 'NSE_EQ|INE091G01018',
    'J&KBANK': 'NSE_EQ|INE168A01041',
    'JINDALSAW': 'NSE_EQ|INE324A01024',
    'JSL': 'NSE_EQ|INE220G01021',
    'JINDALSTEL': 'NSE_EQ|INE749A01030',
    'JIOFIN': 'NSE_EQ|INE758E01017',
    'JUBLFOOD': 'NSE_EQ|INE797F01020',
    'JUBLINGREA': 'NSE_EQ|INE0BY001018',
    'JUBLPHARMA': 'NSE_EQ|INE700A01033',
    'JWL': 'NSE_EQ|INE209L01016',
    'JUSTDIAL': 'NSE_EQ|INE599M01018',
    'JYOTHYLAB': 'NSE_EQ|INE668F01031',
    'KPRMILL': 'NSE_EQ|INE930H01031',
    'KEI': 'NSE_EQ|INE878B01027',
    'KNRCON': 'NSE_EQ|INE634I01029',
    'KPITTECH': 'NSE_EQ|INE04I401011',
    'KRBL': 'NSE_EQ|INE001B01026',
    'KSB': 'NSE_EQ|INE999A01015',
    'KAJARIACER': 'NSE_EQ|INE217B01036',
    'KPIL': 'NSE_EQ|INE220B01022',
    'KALYANKJIL': 'NSE_EQ|INE303R01014',
    'KANSAINER': 'NSE_EQ|INE531A01024',
    'KARURVYSYA': 'NSE_EQ|INE036D01028',
    'KAYNES': 'NSE_EQ|INE918Z01012',
    'KEC': 'NSE_EQ|INE389H01022',
    'KFINTECH': 'NSE_EQ|INE0NNS01018',
    'KOTAKBANK': 'NSE_EQ|INE237A01028',
    'KIMS': 'NSE_EQ|INE967H01017',
    'LTF': 'NSE_EQ|INE498L01015',
    'LTTS': 'NSE_EQ|INE010V01017',
    'LICHSGFIN': 'NSE_EQ|INE115A01026',
    'LTIM': 'NSE_EQ|INE214T01019',
    'LT': 'NSE_EQ|INE018A01030',
    'LATENTVIEW': 'NSE_EQ|INE0I7C01011',
    'LAURUSLABS': 'NSE_EQ|INE947Q01028',
    'LXCHEM': 'NSE_EQ|INE576O01020',
    'LEMONTREE': 'NSE_EQ|INE970X01018',
    'LICI': 'NSE_EQ|INE0J1Y01017',
    'LINDEINDIA': 'NSE_EQ|INE473A01011',
    'LLOYDSME': 'NSE_EQ|INE281B01032',
    'LUPIN': 'NSE_EQ|INE326A01037',
    'MMTC': 'NSE_EQ|INE123F01029',
    'MRF': 'NSE_EQ|INE883A01011',
    'MTARTECH': 'NSE_EQ|INE864I01014',
    'LODHA': 'NSE_EQ|INE670K01029',
    'MGL': 'NSE_EQ|INE002S01010',
    'MAHSEAMLES': 'NSE_EQ|INE271B01025',
    'M&MFIN': 'NSE_EQ|INE774D01024',
    'M&M': 'NSE_EQ|INE101A01026',
    'MHRIL': 'NSE_EQ|INE998I01010',
    'MAHLIFE': 'NSE_EQ|INE813A01018',
    'MANAPPURAM': 'NSE_EQ|INE522D01027',
    'MRPL': 'NSE_EQ|INE103A01014',
    'MANKIND': 'NSE_EQ|INE634S01028',
    'MARICO': 'NSE_EQ|INE196A01026',
    'MARUTI': 'NSE_EQ|INE585B01010',
    'MASTEK': 'NSE_EQ|INE759A01021',
    'MFSL': 'NSE_EQ|INE180A01020',
    'MAXHEALTH': 'NSE_EQ|INE027H01010',
    'MAZDOCK': 'NSE_EQ|INE249Z01012',
    'MEDPLUS': 'NSE_EQ|INE804L01022',
    'METROBRAND': 'NSE_EQ|INE317I01021',
    'METROPOLIS': 'NSE_EQ|INE112L01020',
    'MINDACORP': 'NSE_EQ|INE842C01021',
    'MSUMI': 'NSE_EQ|INE0FS801015',
    'MOTILALOFS': 'NSE_EQ|INE338I01027',
    'MPHASIS': 'NSE_EQ|INE356A01018',
    'MCX': 'NSE_EQ|INE745G01035',
    'MUTHOOTFIN': 'NSE_EQ|INE414G01012',
    'NATCOPHARM': 'NSE_EQ|INE987B01026',
    'NBCC': 'NSE_EQ|INE095N01031',
    'NCC': 'NSE_EQ|INE868B01028',
    'NHPC': 'NSE_EQ|INE848E01016',
    'NLCINDIA': 'NSE_EQ|INE589A01014',
    'NMDC': 'NSE_EQ|INE584A01023',
    'NSLNISP': 'NSE_EQ|INE0NNS01018',
    'NTPC': 'NSE_EQ|INE733E01010',
    'NH': 'NSE_EQ|INE410P01011',
    'NATIONALUM': 'NSE_EQ|INE139A01034',
    'NAVINFLUOR': 'NSE_EQ|INE048G01026',
    'NESTLEIND': 'NSE_EQ|INE239A01024',
    'NETWORK18': 'NSE_EQ|INE870H01013',
    'NAM-INDIA': 'NSE_EQ|INE298J01013',
    'NUVAMA': 'NSE_EQ|INE531F01015',
    'NUVOCO': 'NSE_EQ|INE118D01016',
    'OBEROIRLTY': 'NSE_EQ|INE093I01010',
    'ONGC': 'NSE_EQ|INE213A01029',
    'OIL': 'NSE_EQ|INE274J01014',
    'OLECTRA': 'NSE_EQ|INE260D01016',
    'PAYTM': 'NSE_EQ|INE982J01020',
    'OFSS': 'NSE_EQ|INE881D01027',
    'POLICYBZR': 'NSE_EQ|INE417T01026',
    'PCBL': 'NSE_EQ|INE602A01031',
    'PIIND': 'NSE_EQ|INE603J01030',
    'PNBHOUSING': 'NSE_EQ|INE572E01012',
    'PNCINFRA': 'NSE_EQ|INE195J01029',
    'PVRINOX': 'NSE_EQ|INE191H01014',
    'PAGEIND': 'NSE_EQ|INE761H01022',
    'PATANJALI': 'NSE_EQ|INE619A01035',
    'PERSISTENT': 'NSE_EQ|INE262H01021',
    'PETRONET': 'NSE_EQ|INE347G01014',
    'PHOENIXLTD': 'NSE_EQ|INE211B01039',
    'PIDILITIND': 'NSE_EQ|INE318A01026',
    'PEL': 'NSE_EQ|INE140A01024',
    'PPLPHARMA': 'NSE_EQ|INE0DK501011',
    'POLYMED': 'NSE_EQ|INE205C01021',
    'POLYCAB': 'NSE_EQ|INE455K01017',
    'POONAWALLA': 'NSE_EQ|INE511C01022',
    'PFC': 'NSE_EQ|INE134E01011',
    'POWERGRID': 'NSE_EQ|INE752E01010',
    'PRAJIND': 'NSE_EQ|INE074A01025',
    'PRESTIGE': 'NSE_EQ|INE811K01011',
    'PRINCEPIPE': 'NSE_EQ|INE689W01016',
    'PRSMJOHNSN': 'NSE_EQ|INE010A01011',
    'PGHH': 'NSE_EQ|INE179A01014',
    'PNB': 'NSE_EQ|INE160A01022',
    'QUESS': 'NSE_EQ|INE615P01015',
    'RRKABEL': 'NSE_EQ|INE777K01022',
    'RBLBANK': 'NSE_EQ|INE976G01028',
    'RECLTD': 'NSE_EQ|INE020B01018',
    'RHIM': 'NSE_EQ|INE743M01012',
    'RITES': 'NSE_EQ|INE320J01015',
    'RADICO': 'NSE_EQ|INE944F01028',
    'RVNL': 'NSE_EQ|INE415G01027',
    'RAILTEL': 'NSE_EQ|INE0DD101019',
    'RAINBOW': 'NSE_EQ|INE961O01016',
    'RAJESHEXPO': 'NSE_EQ|INE343B01030',
    'RKFORGE': 'NSE_EQ|INE399G01023',
    'RCF': 'NSE_EQ|INE027A01015',
    'RATNAMANI': 'NSE_EQ|INE703B01027',
    'RTNINDIA': 'NSE_EQ|INE834M01019',
    'RAYMOND': 'NSE_EQ|INE301A01014',
    'REDINGTON': 'NSE_EQ|INE891D01026',
    'RELIANCE': 'NSE_EQ|INE002A01018',
    'RBA': 'NSE_EQ|INE07T201019',
    'ROUTE': 'NSE_EQ|INE450U01017',
    'SBFC': 'NSE_EQ|INE423Y01016',
    'SBICARD': 'NSE_EQ|INE018E01016',
    'SBILIFE': 'NSE_EQ|INE123W01016',
    'SJVN': 'NSE_EQ|INE002L01015',
    'SKFINDIA': 'NSE_EQ|INE640A01023',
    'SRF': 'NSE_EQ|INE647A01010',
    'SAFARI': 'NSE_EQ|INE429E01023',
    'SAMMAANCAP': 'NSE_EQ|INE148I01020',
    'MOTHERSON': 'NSE_EQ|INE775A01035',
    'SANOFI': 'NSE_EQ|INE058A01010',
    'SAPPHIRE': 'NSE_EQ|INE806T01012',
    'SAREGAMA': 'NSE_EQ|INE979A01025',
    'SCHAEFFLER': 'NSE_EQ|INE513A01022',
    'SCHNEIDER': 'NSE_EQ|INE839M01018',
    'SHREECEM': 'NSE_EQ|INE070A01015',
    'RENUKA': 'NSE_EQ|INE087H01022',
    'SHRIRAMFIN': 'NSE_EQ|INE721A01013',
    'SHYAMMETL': 'NSE_EQ|INE810G01011',
    'SIEMENS': 'NSE_EQ|INE003A01024',
    'SIGNATURE': 'NSE_EQ|INE903U01023',
    'SOBHA': 'NSE_EQ|INE671H01015',
    'SOLARINDS': 'NSE_EQ|INE343H01029',
    'SONACOMS': 'NSE_EQ|INE073K01018',
    'SONATSOFTW': 'NSE_EQ|INE269A01021',
    'STARHEALTH': 'NSE_EQ|INE575P01011',
    'SBIN': 'NSE_EQ|INE062A01020',
    'SAIL': 'NSE_EQ|INE114A01011',
    'SWSOLAR': 'NSE_EQ|INE00M201021',
    'STLTECH': 'NSE_EQ|INE089C01029',
    'SUMICHEM': 'NSE_EQ|INE258G01013',
    'SPARC': 'NSE_EQ|INE232I01014',
    'SUNPHARMA': 'NSE_EQ|INE044A01036',
    'SUNTV': 'NSE_EQ|INE424H01027',
    'SUNDARMFIN': 'NSE_EQ|INE660A01013',
    'SUNDRMFAST': 'NSE_EQ|INE387A01021',
    'SUNTECK': 'NSE_EQ|INE805D01034',
    'SUPREMEIND': 'NSE_EQ|INE195A01028',
    'SUVENPHAR': 'NSE_EQ|INE03QK01018',
    'SUZLON': 'NSE_EQ|INE040H01021',
    'SWANENERGY': 'NSE_EQ|INE665A01038',
    'SYNGENE': 'NSE_EQ|INE398R01022',
    'SYRMA': 'NSE_EQ|INE0DYJ01015',
    'TBOTEK': 'NSE_EQ|INE0J5401028',
    'TVSMOTOR': 'NSE_EQ|INE494B01023',
    'TVSSCS': 'NSE_EQ|INE395N01027',
    'TMB': 'NSE_EQ|INE668A01016',
    'TANLA': 'NSE_EQ|INE483C01032',
    'TATACHEM': 'NSE_EQ|INE092A01019',
    'TATACOMM': 'NSE_EQ|INE151A01013',
    'TCS': 'NSE_EQ|INE467B01029',
    'TATACONSUM': 'NSE_EQ|INE192A01025',
    'TATAELXSI': 'NSE_EQ|INE670A01012',
    'TATAINVEST': 'NSE_EQ|INE672A01018',
    'TATAMOTORS': 'NSE_EQ|INE155A01022',
    'TATAPOWER': 'NSE_EQ|INE245A01021',
    'TATASTEEL': 'NSE_EQ|INE081A01020',
    'TATATECH': 'NSE_EQ|INE142M01025',
    'TTML': 'NSE_EQ|INE517B01013',
    'TECHM': 'NSE_EQ|INE669C01036',
    'TEJASNET': 'NSE_EQ|INE010J01012',
    'NIACL': 'NSE_EQ|INE470Y01017',
    'RAMCOCEM': 'NSE_EQ|INE331A01037',
    'THERMAX': 'NSE_EQ|INE152A01029',
    'TIMKEN': 'NSE_EQ|INE325A01013',
    'TITAGARH': 'NSE_EQ|INE615H01020',
    'TITAN': 'NSE_EQ|INE280A01028',
    'TORNTPHARM': 'NSE_EQ|INE685A01028',
    'TORNTPOWER': 'NSE_EQ|INE813H01021',
    'TRENT': 'NSE_EQ|INE849A01020',
    'TRIDENT': 'NSE_EQ|INE064C01022',
    'TRIVENI': 'NSE_EQ|INE256C01024',
    'TRITURBINE': 'NSE_EQ|INE152M01016',
    'TIINDIA': 'NSE_EQ|INE974X01010',
    'UCOBANK': 'NSE_EQ|INE691A01018',
    'UNOMINDA': 'NSE_EQ|INE405E01023',
    'UPL': 'NSE_EQ|INE628A01036',
    'UTIAMC': 'NSE_EQ|INE094J01016',
    'UJJIVANSFB': 'NSE_EQ|INE551W01018',
    'ULTRACEMCO': 'NSE_EQ|INE481G01011',
    'UNIONBANK': 'NSE_EQ|INE692A01016',
    'UBL': 'NSE_EQ|INE686F01025',
    'UNITDSPR': 'NSE_EQ|INE854D01024',
    'USHAMART': 'NSE_EQ|INE228A01035',
    'VGUARD': 'NSE_EQ|INE951I01027',
    'VIPIND': 'NSE_EQ|INE054A01027',
    'VAIBHAVGBL': 'NSE_EQ|INE884A01027',
    'VTL': 'NSE_EQ|INE825A01020',
    'VARROC': 'NSE_EQ|INE665L01035',
    'VBL': 'NSE_EQ|INE200M01021',
    'MANYAVAR': 'NSE_EQ|INE825V01034',
    'VEDL': 'NSE_EQ|INE205A01025',
    'VIJAYA': 'NSE_EQ|INE043W01024',
    'IDEA': 'NSE_EQ|INE669E01016',
    'VOLTAS': 'NSE_EQ|INE226A01021',
    'WELCORP': 'NSE_EQ|INE191B01025',
    'WELSPUNLIV': 'NSE_EQ|INE192B01031',
    'WESTLIFE': 'NSE_EQ|INE274F01020',
    'WHIRLPOOL': 'NSE_EQ|INE716A01013',
    'WIPRO': 'NSE_EQ|INE075A01022',
    'YESBANK': 'NSE_EQ|INE528G01035',
    'ZFCVINDIA': 'NSE_EQ|INE342J01019',
    'ZEEL': 'NSE_EQ|INE256A01028',
    'ZENSARTECH': 'NSE_EQ|INE520A01027',
    'ETERNAL': 'NSE_EQ|INE758T01015',
    'ZYDUSLIFE': 'NSE_EQ|INE010B01027',
    'ECLERX': 'NSE_EQ|INE738I01010',
    'NIFTY': 'NSE_INDEX|NSE_INDEX',

    # NSE ETFs (complete list)
    'ABSLBANETF': 'NSE_EQ|INF209KB17D5',
    'ALPHAETF': 'NSE_EQ|INF769K01KU7',
    'ALPL30IETF': 'NSE_EQ|INF109KC17V7',
    'AUTOBEES': 'NSE_EQ|INF204KC1337',
    'AUTOIETF': 'NSE_EQ|INF109KC10V2',
    'AXISBNKETF': 'NSE_EQ|INF846K01X63',
    'AXISBPSETF': 'NSE_EQ|INF846K01Z04',
    'AXISCETF': 'NSE_EQ|INF846K016C7',
    'AXISGOLD': 'NSE_EQ|INF846K01W80',
    'AXISHCETF': 'NSE_EQ|INF846K01Z12',
    'AXISTECETF': 'NSE_EQ|INF846K01Y96',
    'BANKBEES': 'NSE_EQ|INF204KB15I9',
    'BANKBETF': 'NSE_EQ|INF0QA701714',
    'BANKETF': 'NSE_EQ|INF769K01KR3',
    'BANKIETF': 'NSE_EQ|INF109KC15I8',
    'BANKNIFTY1': 'NSE_EQ|INF174K01F59',
    'BBETF0432': 'NSE_EQ|INF754K01OB1',
    'BBNPNBETF': 'NSE_EQ|INF251K01TL6',
    'BFSI': 'NSE_EQ|INF769K01HI8',
    'BSE500IETF': 'NSE_EQ|INF109KC1V59',
    'BSLGOLDETF': 'NSE_EQ|INF209KB18D3',
    'BSLNIFTY': 'NSE_EQ|INF209KB19D1',
    'BSLSENETFG': 'NSE_EQ|INF209KB10E8',
    'CASHIETF': 'NSE_EQ|INF109K1A021',
    'COMMOIETF': 'NSE_EQ|INF109KC19O8',
    'CONSUMBEES': 'NSE_EQ|INF204KA1LD7',
    'CONSUMIETF': 'NSE_EQ|INF109KC1V42',
    'CPSEETF': 'NSE_EQ|INF457M01133',
    'EBBETF0430': 'NSE_EQ|INF754K01KO2',
    'EBBETF0431': 'NSE_EQ|INF754K01LE1',
    'EBBETF0433': 'NSE_EQ|INF754K01QX0',
    'EQUAL50ADD': 'NSE_EQ|INF740KA1QK2',
    'ESG': 'NSE_EQ|INF769K01GS9',
    'EVIETF': 'NSE_EQ|INF109K1A153',
    'FINIETF': 'NSE_EQ|INF109KC17L8',
    'FMCGIETF': 'NSE_EQ|INF109KC19V3',
    'GILT5YBEES': 'NSE_EQ|INF204KC1030',
    'GILTIETF': 'NSE_EQ|INF109KC14A8',
    'GOLD1': 'NSE_EQ|INF174KA1HJ8',
    'GOLDBEES': 'NSE_EQ|INF204KB17I5',
    'GOLDETF': 'NSE_EQ|INF769K01JP9',
    'GOLDIETF': 'NSE_EQ|INF109KC1NT3',
    'GOLDSHARE': 'NSE_EQ|INF789F1AUX7',
    'GSEC10IETF': 'NSE_EQ|INF109KC18O0',
    'GSEC10YEAR': 'NSE_EQ|INF769K01KF8',
    'HANGSENG BEES-NAV': 'NSE_INDEX|HangSeng BeES-NAV',
    'HDFCGOLD': 'NSE_EQ|INF179KC1981',
    'HEALTHIETF': 'NSE_EQ|INF109KC1Q72',
    'HNGSNGBEES': 'NSE_EQ|INF204KB19I1',
    'INFRAIETF': 'NSE_EQ|INF109KC16E5',
    'ITBEES': 'NSE_EQ|INF204KB15V2',
    'ITIETF': 'NSE_EQ|INF109KC16I6',
    'ITETF': 'NSE_EQ|INF769K01KV5',
    'IVZINGOLD': 'NSE_EQ|INF205K01361',
    'IVZINNIFTY': 'NSE_EQ|INF205K01DA9',
    'JETFREIGHT': 'NSE_EQ|INE982V01025',
    'JUNIORBEES': 'NSE_EQ|INF732E01045',
    'LICMFGOLD': 'NSE_EQ|INF397L01554',
    'LICNETFGSC': 'NSE_EQ|INF767K01MV5',
    'LICNETFN50': 'NSE_EQ|INF767K01OS7',
    'LICNETFSEN': 'NSE_EQ|INF767K01OT5',
    'LIQGRWBEES': 'NSE_EQ|INF204KC1FU1',
    'LIQUIDBEES': 'NSE_EQ|INF732E01037',
    'LIQUIDBETF': 'NSE_EQ|INF0QA701854',
    'LIQUIDETF': 'NSE_EQ|INF740KA1EU7',
    'LIQUIDIETF': 'NSE_EQ|INF109KC1KT9',
    'LOWVOLIETF': 'NSE_EQ|INF109KC19U5',
    'LTGILTBEES': 'NSE_EQ|INF204KB1882',
    'MAKEINDIA': 'NSE_EQ|INF769K01IB1',
    'MANUFGBEES': 'NSE_EQ|INF204KC1GH6',
    'METALIETF': 'NSE_EQ|INF109KC19W1',
    'MID150BEES': 'NSE_EQ|INF204KB1V68',
    'MIDCAPETF': 'NSE_EQ|INF769K01IC9',
    'MIDCAPIETF': 'NSE_EQ|INF109KC11W8',
    'MIDQ50ADD': 'NSE_EQ|INF740KA1QL0',
    'MIDSELIETF': 'NSE_EQ|INF109KC10W0',
    'MOM100': 'NSE_EQ|INF247L01023',
    'MOM30IETF': 'NSE_EQ|INF109KC17C7',
    'MOM50': 'NSE_EQ|INF247L01536',
    'NETF': 'NSE_EQ|INF277K015R5',
    'NEXT50IETF': 'NSE_EQ|INF109KC1NS5',
    'NIF100BEES': 'NSE_EQ|INF204K014N5',
    'NIF100IETF': 'NSE_EQ|INF109KC16V9',
    'NIF10GETF': 'NSE_EQ|INF789F1AZF3',
    'NIF5GETF': 'NSE_EQ|INF789F1AZE6',
    'NIFITETF': 'NSE_EQ|INF789F1AZF3',
    'NIFTY1': 'NSE_EQ|INF174K014P6',
    'NIFTYBETF': 'NSE_EQ|INF0QA701722',
    'NIFTYBEES': 'NSE_EQ|INF204KB14I2',
    'NIFTYIETF': 'NSE_EQ|INF109K012R6',
    'NIFTYETF': 'NSE_EQ|INF769K01EG9',
    'NV20': 'NSE_EQ|INF174K01Z71',
    'NV20BEES': 'NSE_EQ|INF204KB18I3',
    'NV20IETF': 'NSE_EQ|INF109KC11V0',
    'OILIETF': 'NSE_EQ|INF109KC18W3',
    'PHARMABEES': 'NSE_EQ|INF204KC1089',
    'PSUBANKADD': 'NSE_EQ|INF740KA1SY9',
    'PSUBNKBEES': 'NSE_EQ|INF204KB16I7',
    'PSUBNKIETF': 'NSE_EQ|INF109KC10S8',
    'PVTBANKADD': 'NSE_EQ|INF740KA1TA7',
    'PVTBANIETF': 'NSE_EQ|INF109KC18U7',
    'QNIFTY': 'NSE_EQ|INF082J01028',
    'QUAL30IETF': 'NSE_EQ|INF109KC18V5',
    'SBIETFCON': 'NSE_EQ|INF200KA1X17',
    'SBIETFIT': 'NSE_EQ|INF200KA1S14',
    'SBIETFPB': 'NSE_EQ|INF200KA1S22',
    'SBIETFQLTY': 'NSE_EQ|INF200KA1WX6',
    'SBILIQETF': 'NSE_EQ|INF200KB1969',
    'SBINEQWETF': 'NSE_EQ|INF200KB1282',
    'SDL26BEES': 'NSE_EQ|INF204KC1022',
    'SENSEXADD': 'NSE_EQ|INF740KA1SZ6',
    'SENSEXETF': 'NSE_EQ|INF769K01KT9',
    'SENSEXIETF': 'NSE_EQ|INF346A01034',
    'SETF10GILT': 'NSE_EQ|INF200KA1JT1',
    'SETFGOLD': 'NSE_EQ|INF200KA16D8',
    'SETFNIF50': 'NSE_EQ|INF200KA1FS1',
    'SETFNIFBK': 'NSE_EQ|INF200KA1580',
    'SETFNN50': 'NSE_EQ|INF200KA1598',
    'SHARIABEES': 'NSE_EQ|INF732E01128',
    'SILVERADD': 'NSE_EQ|INF740KA1RE3',
    'SILVERBEES': 'NSE_EQ|INF204KC1402',
    'SILVERETF': 'NSE_EQ|INF789F1AYK6',
    'SILVERIETF': 'NSE_EQ|INF109KC1Y56',
    'SNXT30BEES': 'NSE_EQ|INF204KC1EZ3',
    'TNIDETF': 'NSE_EQ|INF277KA1364',
    'TOP15IETF': 'NSE_EQ|INF109K1A344',
    'UTIBANKETF': 'NSE_EQ|INF789F1AUV1',
    'UTINIFTETF': 'NSE_EQ|INF789F1AZC0',
    'UTISENSETF': 'NSE_EQ|INF789FB1X58',
    'VAL30IETF': 'NSE_EQ|INF109KC16X5'
}

# NIFTY 500 stocks list (with .NS suffix for compatibility)
# Dynamic universe loading
NIFTY_500_STOCKS: List[str] = fetch_nifty200_stocks()  # Now fetches NIFTY 200
NSE_ETFS: List[str] = fetch_nse_etfs()

# Combined list of all instruments
ALL_INSTRUMENTS: List[str] = NIFTY_500_STOCKS + NSE_ETFS